...
import static generic.Simulator.getEventQueue;
...
public class InstructionFetch implements Element{
		...
		if(IF_EnableLatch.isIF_enable())
		{
			...
			if(IF_EnableLatch.stall_switch())
			{
				if(!IF_EnableLatch.isIF_busy())
				{
					...
					getEventQueue().addEvent(new CacheReadEvent_i(getCurrentTime()+Configuration.L1i_latency,this,containingProcessor.getCache_I(),currentPC));
					setIFbusytotrue();
					disableOFstage();
					disableEXstage();
					disableMAstage();
	...
	public void handleEvent(Event e)
	{
		...
		else
		{
			int currentPC;
			currentPC = containingProcessor.getRegisterFile().getProgramCounter();
			CacheResponseEvent_i event;
			event = (CacheResponseEvent_i) e;
			IF_OF_Latch.setInstruction(event.getValue());
			containingProcessor.getRegisterFile().setProgramCounter(currentPC + 1);
			increment_dynamic_instructions();
